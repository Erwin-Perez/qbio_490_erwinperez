...
clinic[1, "age_at_diagnosis"]
clinic[1, "last_contact_days_to"]

# 50 + (3767/360) = 50 + 10 


# 1. There are 1097 rows and 112 columns. 
# 2. Character data type (chr)
# 3. 50 years old.
# 4. 60 years old
```
...
```{r}
under30_over60_mask <- (clinic$age_at_diagnosis > 60) | (clinic$age_at_diagnosis < 35)
```
...
```{r}
cleaned_clinic$survival_time <- ifelse(cleaned_clinic$last_contact_days_to == "[Not Available]", cleaned_clinic$death_days_to, cleaned_clinic$last_contact_days_to)
```
...
# 30% (assuming 15 years is ~5000 days)
...
# P-value was 0.2 meaning that there is not enough evidence to support statistically significant effect. Though, I assume that there might be an error in my code since it should be < 0.05 in which that would tell us that there is a statistical signifiance in that increasing age leads to worse breast cancer survivability. 
